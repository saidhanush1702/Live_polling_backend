...
// --- Helper: format poll for clients ---
function pollToClient(pollDoc) {
    ...
    return {
        ...
        correctIndex: pollDoc.correctIndex, // send correct index if needed for teacher
...
io.on("connection", (socket) => {
    ...
    socket.on("join", ({ name, role }) => {
        if (role === "student") {
            ...
            io.emit(
                "updateParticipants",
                Array.from(connectedStudents.values()).map((s) => s.name)
            );
        } else if (role === "teacher") {
            // Send teacher the current active poll
            if (currentPoll) socket.emit("newPoll", pollToClient(currentPoll));
            // ALSO send the current list of connected students
            socket.emit(
                "updateParticipants",
                Array.from(connectedStudents.values()).map((s) => s.name)
            );
    ...
    socket.on('createPoll', async ({ question, options, duration = 60, correctIndex }) => {
        try {
            if (!question || !options || options.length < 2 || correctIndex == null) {
    ...
    socket.on('kickStudent', (data) => {
        ...
        const entry = Array.from(connectedStudents.entries()).find(
...
app.post('/api/polls', async (req, res) => {
    try {
        const { question, options, duration = 60, correctIndex } = req.body;
        if (!question || !options || options.length < 2 || correctIndex == null) {
            return res.status(400).json({ error: 'Poll must have a question, at least 2 options, and a correct answer' });
        }
        if (currentPoll && !currentPoll.ended) {
